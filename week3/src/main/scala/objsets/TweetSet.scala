  ...
  def descendingByRetweet: TweetList = {
    ...
    new TweetList {
      override def tail: TweetList = {
        val removedTweetSet = remove(tweet)
        if (!removedTweetSet.isInstanceOf[Empty]) removedTweetSet.descendingByRetweet
        else Nil
      }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    val leftFilter = left.filter(tweet => tweet.retweets > elem.retweets)
    if (leftFilter.isInstanceOf[Empty]) {
      val rightFilter = right.filter(tweet => tweet.retweets > elem.retweets)
      if (rightFilter.isInstanceOf[Empty]) elem
      else rightFilter.mostRetweeted
    } else {
      val maxLeftRetweeted = leftFilter.mostRetweeted
      val rightFilter = right.filter(tweet => tweet.retweets > maxLeftRetweeted.retweets)
      if (rightFilter.isInstanceOf[Empty]) maxLeftRetweeted
      else rightFilter.mostRetweeted
    }
  }
